    ...
    public class SteeringRig2DEditor : Editor
    {
        ...
        void displayErrors()
        {
            ...
            else
            {
                ...
                {
                    if (raySensors[i].IgnoreList.Length > 0 && raySensors[i].IgnoreList != steeringRig.IgnoreList)
                    {
        ...
        {
            return PrefabUtility.GetPrefabParent(steeringRig) == null || steeringRig.GetComponentsInChildren<RaySensor2D>().Length > 0;
        }
    }
}