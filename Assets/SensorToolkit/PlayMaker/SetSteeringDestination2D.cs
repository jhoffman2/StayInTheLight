...
namespace SensorToolkit.PlayMaker
{
    ...
    public class SetSteeringDestination2D : SensorToolkitComponentAction<SteeringRig2D>
    {
        ...
        void doCheck()
        {
            ...
            {
                steeringRig2D.DestinationTransform = destinationGameObject.Value.transform;
            }
            else
            {
                steeringRig2D.DestinationTransform = null;
            }
            if (!destinationPosition.IsNone)
            {
                steeringRig2D.Destination = destinationPosition.Value;
            }
        }
    }
}
